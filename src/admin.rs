...
async fn save_config(Form(config): Form<AppConfig>) -> impl IntoResponse {
	match framework_save_config(&config) {
		...
		Err(e) => Html(format!(
			"<span style='color: red;'>Ошибка сохранения: {}</span>",
			e
		)),
	}
}
...
async fn dispatch_request(
	...
	let plugin_arc = {
		...
		match modules.get(&module_name) {
			Some(p) => p.clone(),
			None => return (StatusCode::NOT_FOUND, format!("Module '{}' not found", module_name)).into_response(),
		}
	};
	...
	let result = module.plugin.call::<&[u8], &[u8]>("handle_request", &serde_json::to_vec(&plugin_req).unwrap());

	match result {
		Ok(resp_bytes) => {
			let plugin_resp: PluginResponse = match serde_json::from_slice(resp_bytes) {
				Ok(r) => r,
				Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, format!("Plugin returned invalid JSON: {}", e)).into_response(),
			};

			// Конвертация PluginResponse в axum::response::Response
			let mut response_builder = Response::builder()
				.status(StatusCode::from_u16(plugin_resp.status).unwrap_or(StatusCode::INTERNAL_SERVER_ERROR));
		...
		if name == "plugin" && file_name.ends_with(".wasm") {
			...
			if !path.exists() {
				...
				if let Err(e) = std::fs::create_dir_all(&path) {
					...
					return (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to create plugins directory: {}", e)).into_response();
				}
			}
			...
			if let Err(e) = std::fs::write(&path, data) {
				...
				return (StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to save module to disk: {}", e)).into_response();
			}
			println!("Module saved to {:?}", path);

			return (StatusCode::OK, format!("Module '{}' successfully uploaded and activated", file_name)).into_response();
		}
	}
